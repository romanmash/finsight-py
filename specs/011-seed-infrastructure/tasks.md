...
- [X] T005 [US2] Create `apps/api-service/src/api/seeds/operators.py` with `async def seed_operators(session: AsyncSession) -> None`: creates admin operator (`email="admin@finsight.local"`, `role="admin"`, `telegram_user_id=<fixed int>`, `telegram_chat_id=<fixed int>`, password hashed with bcrypt from `SEED_ADMIN_PASSWORD`) and viewer operator (`email="viewer@finsight.local"`, `role="viewer"`, password from `SEED_VIEWER_PASSWORD`) using `session.merge()`; all timestamps fixed to past datetime
