...
- [ ] T015 [US3] Create `infra/pulumi/Pulumi.yaml` with `name: finsight`, `runtime: python`, `description: FinSight AI Hub infrastructure`
- [ ] T016 [P] [US3] Create `infra/pulumi/Pulumi.dev.yaml` with dev stack config: `finsight:server_type: cx21`, `finsight:region: nbg1`, `finsight:volume_size_gb: 20`
- [ ] T017 [P] [US3] Create `infra/pulumi/requirements.txt` with `pulumi>=3.0`, `pulumi-hcloud` — do NOT add to uv workspace; Pulumi manages its own `.venv`
...
- [ ] T019 [US3] Add `PULUMI_SSH_PUBLIC_KEY` and `HCLOUD_TOKEN` to `.env.example` with placeholder values and comments

---

## Phase 7: GitHub Actions CI

- [ ] T020 Create `.github/workflows/ci-cd.yml` with trigger `on: push: branches: [main]`; single job `ci` on `ubuntu-latest`; steps: `actions/checkout@v4`, `astral-sh/setup-uv@v3` with `python-version: "3.13"`, `uv sync --all-packages`, `uv run mypy --strict apps/ packages/`, `uv run ruff check apps/ packages/`, `uv run pytest --tb=short -q`; add `UV_CACHE_DIR` env for caching
