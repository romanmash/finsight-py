...
- [ ] T001 Create `apps/api/src/api/agents/__init__.py` (empty) and `apps/api/src/api/agents/shared/__init__.py` (empty) in `apps/api/src/api/agents/`
- [ ] T002 [P] Create `apps/api/tests/agents/__init__.py` (empty) in `apps/api/tests/agents/__init__.py`
- [ ] T003 [P] Add `langchain-core`, `langchain-openai`, `langsmith` to `apps/api/pyproject.toml` dependencies (if not already present) in `apps/api/pyproject.toml`
...
- [ ] T004 Update `config/runtime/agents.yaml` to add `fallback_model` and `fallback_provider` fields to each of the 7 agent definitions (manager, watchdog, researcher, analyst, pattern_specialist, bookkeeper, reporter); keep existing fields (model, provider, temperature, max_tokens, max_retries, timeout_seconds, base_url) in `config/runtime/agents.yaml`
...
- [ ] T006 Create `apps/api/src/api/lib/pricing.py` with `PricingRegistry`:
  ...
  in `apps/api/src/api/lib/pricing.py`
- [ ] T007 [P] Create `apps/api/src/api/lib/tracing.py` with LangSmith tracing wrapper:
  ...
  in `apps/api/src/api/lib/tracing.py`
- [ ] T008 Create `apps/api/tests/agents/conftest.py` with shared fixtures for all agent tests:
  ...
  - `agent_configs` — `AgentsConfig` loaded from test YAML fixture with all 7 agents
  in `apps/api/tests/agents/conftest.py`
...
**Independent Test**: `uv run pytest apps/api/tests/agents/test_base.py::test_run_creates_agent_run_record` — passes offline.

- [ ] T009 [US1] Create `apps/api/src/api/agents/base.py` with `BaseAgent[InputT, OutputT]` abstract class:
  ...
  in `apps/api/src/api/agents/base.py`
- [ ] T010 [US1] Write US1 tests in `apps/api/tests/agents/test_base.py`:
  ...
  in `apps/api/tests/agents/test_base.py`

**Checkpoint**: `uv run pytest apps/api/tests/agents/test_base.py -k "run_creates or cost or tracing or unknown"` — all 4 pass offline.
...
**Independent Test**: `uv run pytest apps/api/tests/agents/test_base.py -k "tool"` — all pass offline.

- [ ] T011 [US2] Add `async def _call_tool(self, tool_name: str, params: dict) -> dict | None` to `BaseAgent` in `apps/api/src/api/agents/base.py`:
  ...
  in `apps/api/src/api/agents/base.py`
- [ ] T012 [US2] Add US2 tests to `apps/api/tests/agents/test_base.py`:
...
**Checkpoint**: `uv run pytest apps/api/tests/agents/test_base.py -k "call_tool"` — all 3 pass offline.
...
**Independent Test**: `uv run pytest apps/api/tests/agents/test_base.py -k "retry or validation or output"` — all pass.

- [ ] T013 [US3] Refactor `BaseAgent.run()` in `apps/api/src/api/agents/base.py` to extract `_invoke_llm_with_retry(prompt, chain) -> OutputT`:
  ...
  in `apps/api/src/api/agents/base.py`
- [ ] T014 [US3] Add US3 tests to `apps/api/tests/agents/test_base.py`:
...
**Checkpoint**: `uv run pytest apps/api/tests/agents/test_base.py -k "retry or valid or invalid or failed"` — all pass offline.
...
- [ ] T015 [US4] Create `apps/api/src/api/agents/shared/prompts.py` with shared prompt fragments:
  ...
  in `apps/api/src/api/agents/shared/prompts.py`
- [ ] T016 [US4] Create stub agent to validate the colocated pattern:
  - `apps/api/src/api/agents/stub_agent.prompt.py` — imports from `agents/shared/prompts.py`; defines `STUB_SYSTEM_PROMPT: str`
  - `apps/api/src/api/agents/stub_agent.py` — `StubAgent(BaseAgent[StubInput, StubOutput])` where `StubInput(BaseModel)` has `query: str` and `StubOutput(BaseModel)` has `answer: str`; `name = "stub"`; `output_schema = StubOutput`; `_build_prompt(input) -> str` loads from `.prompt.py`
  in `apps/api/src/api/agents/stub_agent.py` and `apps/api/src/api/agents/stub_agent.prompt.py`
- [ ] T017 [P] [US4] Add US4 tests to `apps/api/tests/agents/test_base.py`:
  - `test_prompt_file_colocated_with_agent` — assert `Path("apps/api/src/api/agents/stub_agent.prompt.py").exists()` is True
...
**Checkpoint**: `uv run pytest apps/api/tests/agents/test_base.py -k "prompt or colocated or stub"` — all pass.

---

## Phase 7: Polish & Cross-Cutting Concerns

- [ ] T018 [P] Add US1 fallback provider test to `apps/api/tests/agents/test_base.py`:
  ...
  in `apps/api/tests/agents/test_base.py`
- [ ] T019 [P] Run `uv run mypy --strict apps/api/src/api/agents/ apps/api/src/api/lib/pricing.py apps/api/src/api/lib/tracing.py` — fix all type errors until zero errors remain
- [ ] T020 Run full test suite: `uv run pytest apps/api/tests/agents/` (all offline, all pass) + `uv run ruff check apps/api/src/api/agents/ apps/api/src/api/lib/pricing.py apps/api/src/api/lib/tracing.py` (zero warnings)
...
4. **STOP and VALIDATE**: `uv run pytest apps/api/tests/agents/test_base.py -k "run_creates or cost"` — pass
