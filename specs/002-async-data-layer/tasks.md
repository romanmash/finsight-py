...
- [ ] T010 [P] Create Pydantic domain model `packages/shared/src/finsight/shared/models/mission.py` with `MissionStatus` enum (PENDING, RUNNING, COMPLETED, FAILED, CANCELLED) and `MissionModel(BaseModel)` matching all Mission columns in `packages/shared/src/finsight/shared/models/mission.py`
