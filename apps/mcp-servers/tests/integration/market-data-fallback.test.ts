...
describe('market-data fallback', () => {
  it('falls back to FMP when Finnhub fails', async () => {
    mswServer.use(
      http.get('https://market-data.test/quote', () => new HttpResponse(null, { status: 500 })),
      http.get('https://fmp.test/quote/NVDA', () => HttpResponse.json([{ price: 222, changesPercentage: 1.2, volume: 9_999, yearHigh: 240, yearLow: 120, marketCap: 600_000_000 }]))
    );
    ...
    const result = await invokeTool<{ output: { price: number } }>(app, {
      tool: 'get_quote',
      input: { ticker: 'NVDA' }
    });

    expect(result.status).toBe(200);
    expect(result.body.output.price).toBe(222);
  });
});
