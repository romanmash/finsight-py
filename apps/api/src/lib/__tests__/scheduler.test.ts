...
vi.mock('../config.js', () => ({
  getConfig: (): { scheduler: { watchdogScan: { cron: string; concurrency: number; retryAttempts: number; retryBackoffMs: number }; screenerScan: { cron: string; concurrency: number; retryAttempts: number; retryBackoffMs: number }; dailyBrief: { cron: string; concurrency: number; retryAttempts: number; retryBackoffMs: number }; earningsCheck: { cron: string; concurrency: number; retryAttempts: number; retryBackoffMs: number }; ticketExpiry: { cron: string; concurrency: number; retryAttempts: number; retryBackoffMs: number } } } => ({
    scheduler: {
...
describe('initScheduler', () => {
  ...
  it('registers repeatable queue schedules and reports dedupe-safe contract', async () => {
