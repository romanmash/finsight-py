...
describe('handler error path', () => {
  it('returns temporaryUnavailable when API call throws', async () => {
    const result = await handleIncomingMessage(
      { text: '/brief', messageId: 1, fromId: 1, username: 'active', chatId: 2 },
      {
        config,
        auth: {
    ...
    expect(result.chunks).toEqual(['⚠️ Service temporarily unavailable. Please try again.']);
  });
});
